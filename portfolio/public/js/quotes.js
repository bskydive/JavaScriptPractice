window.$(document).ready(function () {
    ...
    var quotes = [
        ...
        ["Юрий Никулин", "http://fit4brain.com/2416",
            ...
            "История из жизни. Гид во время экскурсии: — А сейчас мы проедем эту большую стену, и вы увидите район, где все бросили пить и курить.",
        ...
        ["Пётр Мамонов", "http://www.adme.ru/vdohnovenie/prostye-pravila-zhizni-ot-petra-mamonova-472805/",
            ...
            "Никогда не ругайте других, не судите о чужих вкусах.",
            "Идите к Богу, ищите Бога. Даже если вы в него не верите.",
            ...
            "Занимайтесь сексом только с любимым человеком.",
            ...
            "Не ходите в кино и на концерты в одиночку.",
            "Всегда говорите спасибо.",
            "Фотографируйте друзей.",
    ...
    function showQuote() {
        ...
        if ($("#id-a-quoteAuthor").text() === quotes[authorInd][0]) {
            authorInd = getRandomNum(0, quotes.length); // repeat randomize for better usability without repeats
        }

        var quoteInd = getRandomNum(2, quotes[authorInd].length - 2); //0 - author, 1 - link
        if ($("#id-p-quoteText").text === quotes[authorInd][quoteInd]) {
            quoteInd = getRandomNum(2, quotes[authorInd].length - 2);// repeat randomize for better usability without repeats
        }

        $("#id-p-quoteText").text(quotes[authorInd][quoteInd]);

        $("#id-a-quoteAuthor").attr("href", quotes[authorInd][1]);
        $("#id-a-quoteAuthor").text(quotes[authorInd][0]);

        var quoteTwitText = quotes[authorInd][quoteInd] + " &hashtags=" + quotes[authorInd][0].replace(/\s+/g, '');

        if (quoteTwitText.length > 140) {
            quoteTwitText = quotes[authorInd][quoteInd].substring(0, 140 - 6 - quotes[authorInd][0].length) + " ..." + " &hashtags=" + quotes[authorInd][0].replace(/\s+/g, '');//-6 because " ..." + " &hashtags="
        }
